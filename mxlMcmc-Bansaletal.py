...
if __name__ == "__main__":
    ...
    dat = np.loadtxt('synth_data.csv', delimiter=',', skiprows=1)
    
    ###
    #Prepare data
    ###
    
    indID = dat[:,0].astype(int)
    obsID = dat[:,1].astype(int)
    altID = dat[:,2].astype(int)
    chosen = dat[:,3].astype(int)
    xFix = dat[:,4:11]
    xRnd = dat[:,11:]
    ...
    xFix_trans = np.array([0, 0, 0, 0, 0, 0, 0])
