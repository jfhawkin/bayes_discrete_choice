...
parameters {
  ...
  vector[PLN] betaln; // hypermeans of the part-worths for random parameters with postivie lognormal priors
  vector[PNLN] betanln; // hypermeans of the part-worths for random parameters with negative lognormal priors
  ...
  cholesky_factor_corr[ASC] L_Omega; // prior correlation
...
transformed parameters {
  ...
  betaln_ind = rep_matrix(betaln', I) + z[,(PN+1):(PN+PLN)]*diag_matrix(tau[(ASC+PN+1):(ASC+PN+PLN)]);
  betanln_ind = rep_matrix(betanln', I) + z[,(PN+PLN+1):(PN+PLN+PNLN)]*diag_matrix(tau[(ASC+PN+PLN+1):(ASC+PN+PLN+PNLN)]);
  beta_ind = rep_matrix(gamma', I) + (diag_pre_multiply(tau[1:ASC],L_Omega) * za)';
}
...
model {
  ...
  betan ~ normal(0, 5);
  betaln[PLN] ~ normal(0.5, 2.5);
  betanln[PNLN] ~ normal(-0.5, 2.5);
  ...
  for(i in 1:I) {
    for(j in 1:T){
        tshift = shift*(j-1);
        utils[1] = beta_ind[i,1] + X[i,(tshift+65)]*betan_ind[i,1] + X[i,(tshift+70)]*betanln_ind[i,1]                   +X[i,(tshift+90)]*betanln_ind[i,2]                                                +X[i,(tshift+80)]*betanln_ind[i,4]                              +X[i,(tshift+105)]*betanln_ind[i,5];
        utils[2] = beta_ind[i,2] + X[i,(tshift+66)]*betan_ind[i,1] + X[i,(tshift+71)]*betanln_ind[i,1]                   +X[i,(tshift+91)]*betanln_ind[i,2]                                                +X[i,(tshift+81)]*betanln_ind[i,4]+X[i,(tshift+102)]*betaln_ind[i,1]+X[i,(tshift+106)]*betanln_ind[i,5];
        utils[3] = beta_ind[i,3] + X[i,(tshift+67)]*betan_ind[i,1] + X[i,(tshift+87)]*betanln_ind[i,1]                   +X[i,(tshift+87)]*betanln_ind[i,2]                   +X[i,(tshift+77)]*betanln_ind[i,3]                                                           +X[i,(tshift+107)]*betanln_ind[i,5];
        utils[4] = beta_ind[i,4] + X[i,(tshift+68)]*betan_ind[i,1] + (X[i,(tshift+73)]+X[i,(tshift+89)])*betanln_ind[i,1]+(X[i,(tshift+89)]+X[i,(tshift+93)])*betanln_ind[i,2]+X[i,(tshift+79)]*betanln_ind[i,3]+X[i,(tshift+83)]*betanln_ind[i,4]+X[i,(tshift+104)]*betaln_ind[i,1]+(X[i,(tshift+108)]+X[i,(tshift+108)])/2*betanln_ind[i,5];
